...
Because this tutorial is focused on DPDK and VPP which make kernel to bypass all the network interfaces resulting in the device having no ethernet access initially. This means we cannot mount NFS when booting. In order to get around it, edit the file `arch/arm64/boot/dts/freescale/fsl-ls1046a-rdb-usdpaa.dts` and remove the block that starts with `ethernet@2` (line 42). This will result in device tree passing the NIC back to the kernel, so it has at least one which it will then use to mount the NFS. The particular one we're removing from it corresponds to RGMII-1 interface on the LS1046A Reference Design board, so we'll have to make sure that u-boot is using it when booting the kernel (we'll get there later on).
...
With this done, let's first set up a TFTP server so that we can always grab a fresh copy of it and the device trees once we're booting our development board.
...
If you followed the progress line by line so far, you should see the build report saying `Build targets in project: 698`. But even if the number is different, it just means that Meson didn't find all the dependencies in the target filesystem, so you can `chroot` back into it and add them if you want, and then re-run the above command. Once you're done, we can now build the sources and install them into place with the following:
