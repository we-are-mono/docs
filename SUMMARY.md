# Table of contents

* [Documentation](README.md)
* [GitHub](https://github.com/orgs/we-are-mono/repositories)

## Gateway development kit

* [Getting started](gateway-development-kit/getting-started.md)
* [Hardware description](gateway-development-kit/hardware-description.md)
* [Flashing firmware](gateway-development-kit/flashing-firmware.md)
* [Installing OPNsense](https://opnsense.mono.si/releases/26.1/)
* [Installing Armbian](gateway-development-kit/installing-armbian.md)
* [Installing OpenWRT](gateway-development-kit/installing_openwrt.md)
* [Boot process](gateway-development-kit/boot-process.md)
* [Hardware offloading (OPNsense/FreeBSD)](https://github.com/we-are-mono/opnsense-deps/blob/master/ARCHITECTURE.md)

## Tutorials

* [Build a Debian Linux image from scratch](tutorials/linux-from-scratch.md)
* [How to set up DPDK and VPP environment for LS1046A](tutorials/development-set-up.md)
